...
namespace DTcms.Web.tools
{
    ...
    public class admin_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        private void manager_validate(HttpContext context)
        {
            ...
            if (bll.IsAdManagerExists(user_name))
            {
        ...
        private void get_navigation_list(HttpContext context)
        {
            ...
            //有权限的用户或超级管理员即可
            if (roleModel.manager_role_values != null || roleModel.role_type == 1)
            {
            ...
            //获得该管理员父级角色的信息
            string parent_id = roleModel.parent_id;
            //递归赋予权限
            while (!"0".Equals(parent_id.Trim()))
            {
                Model.manager_role parentRoleModel = new BLL.manager_role().GetModel(parent_id);
                if (parentRoleModel != null && parentRoleModel.manager_role_values != null)
                {
                    this.get_navigation_childs_parent(flag, context, dt, 0, "", parentRoleModel.role_type, parentRoleModel.manager_role_values);
                    //this.get_navigation_childs(context, dt, 0, "", roleModel.role_type, parentRoleModel.manager_role_values);
                    return;
                }
                parent_id = parentRoleModel.parent_id;
            }
        }
        ...
        /// <param name="ls"></param>
        private void get_navigation_childs_parents(HttpContext context, DataTable oldData, int parent_id, string parent_name, int role_type, List<Model.manager_role_value> ls)
        {
        ...
        /// <param name="role_type"></param>
        /// <param name="ls"></param>
        private void get_navigation_childs_parent(bool flag,HttpContext context, DataTable oldData, int parent_id, string parent_name, int role_type,List<Model.manager_role_value> ls)
        {
            DataRow[] dr = oldData.Select("parent_id=" + parent_id);
            bool isWrite = false;
            for (int i = 0; i < dr.Length; i++)
            {
                ...
                //以下是输出选项内容=======================
                if (int.Parse(dr[i]["class_layer"].ToString()) == 1 && flag)
                {
                    context.Response.Write("<div class=\"list-group\" name=\"" + dr[i]["sub_title"].ToString() + "\">\n");
                    if (dr[i]["name"].ToString() != "sys_contents")
                    {
                    ...
                    //调用自身迭代
                    this.get_navigation_childs_parent(flag, context, oldData, int.Parse(dr[i]["id"].ToString()), dr[i]["name"].ToString(), role_type, ls);
                    //this.get_navigation_childs(context, oldData, int.Parse(dr[i]["id"].ToString()), dr[i]["name"].ToString(), role_type, ls);
                    context.Response.Write("</div>\n");
                }
                else if (int.Parse(dr[i]["class_layer"].ToString()) == 2 && parent_name == "sys_contents")
